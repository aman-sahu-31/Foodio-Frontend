...
function OwnerRestaurantManagement({ userId }) {
  ...
  const handleCreateRestaurant = async () => {
    try {
      ...
      const newRestaurant = {
        ...
        foodType: createData.foodType.split(",").map((f) => f.trim()),
      ...
      // Add new restaurant immediately to state
...
const handleSave = async () => {
  try {
    ...
    // Use the updated restaurant from backend
    setRestaurants((prev) =>
...
const handleDelete = async (id) => {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {/* Create Form */}
      {showCreateForm && (
        <div className="p-6 bg-white rounded shadow mb-6 space-y-4">
          <h3 className="text-lg font-semibold">Create New Restaurant</h3>
          <div className="grid md:grid-cols-2 gap-4">
            {Object.keys(initialCreateData).map((field, idx) =>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {restaurants.map((resto) => (
          <div key={resto._id} className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition">
            <img
              ...
              {editingId === resto._id ? (
                ...
                <>
                  ...
                  <p className="text-gray-500 text-sm mt-1">
                    Timing: {resto.timing?.open} - {resto.timing?.close} ({resto.timing?.days?.join(", ")})
                  </p>
                  <p className="text-gray-500 text-sm mt-1">
                    Facilities:{" "}
