        ...
        {item.Data?.categories && Object.keys(item.Data.categories).length > 0 && (
          <aside className="md:w-1/4 bg-orange-50 rounded-2xl shadow-md p-4 h-fit sticky top-6">
            ...
            <ul className="space-y-2 text-sm text-gray-800">
              {Object.entries(item.Data.categories).map(([category, items], index) => (
                <li key={index}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {item.Data.maggiItems.map((maggi, index) => (
                  <div key={index} className="relative bg-white rounded-2xl shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition">
                    ...
                    <NavLink to={"/addOnscard/${item.id}"}>                        <button className="bg-purple-100 text-purple-700 text-sm font-semibold px-4 py-1 rounded hover:bg-purple-200">Add</button>
                     </NavLink>
